...
namespace Avora
{
    ...
    public partial class App : Application
    {
        ...
        {
            services.AddSingleton<IWebProxy>(_ =>
            {
                try
                {
                    var proxy = WebRequest.DefaultWebProxy;
                    if (proxy != null && proxy.GetProxy(new Uri("https://api.vk.com")) != null)
                        return proxy;
                }
                catch { }
                return null;
        ...
        public App()
        {
            ...
            if (savedTheme != null && savedTheme.settingValue == "light")
            {
        ...
        protected override async void OnLaunched(Microsoft.UI.Xaml.LaunchActivatedEventArgs args)
        {
            ...
            const string mutexName = "AvoraApps";
            bool createdNew;
                            ...
                            unsafe
                            {
                                fixed (char* pBuffer = buffer)
                                {
                                    ...
                                    if (copiedChars > 0)
                                    {
                                        ...
                                        // Проверяем, содержит ли заголовок "Avora" (или точное совпадение)
                                        if (windowTitle.Contains("Avora")) // или windowTitle == "Avora"
                                        {
