...
namespace Avora.VKs
{
    internal class VKAuth :VK
    {
        ...
        {
            var result = await _authCategory.GetAuthCodeAsync("Avora Player", forceRegenerate);
        ...
        */
        ...
        private async Task AuthPasskeyAsync()
        {
            ...
            if (hResult.Succeeded)
            {
                ...
                if (!hResult.Succeeded)
                {
                    _ = login.DialogMessageShow("Не удалось отменить текущий процесс входа, Avora не смог отменить текущий процесс входа! Закройте все диалоги входа с ключем и повторите попытку.");
