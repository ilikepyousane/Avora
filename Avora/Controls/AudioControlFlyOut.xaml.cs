...
namespace Avora.Controls
{
    public sealed partial class AudioControlFlyOut : MenuFlyout
    {
        ...
        public async void AddRemove_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var newId = await vkService.AudioAddAsync((long)dataTrack.audio.Id, (long)dataTrack.audio.OwnerId);
                this.DispatcherQueue.TryEnqueue(() =>
                {
                    dataTrack.audio.OwnerId = (long)AccountsDB.activeAccount.id;
                    dataTrack.trackOwnerChangedInvoke();
                    updateUIFlyOut();
                });
            }
        }
