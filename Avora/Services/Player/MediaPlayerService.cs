...
namespace Avora.Services
{
    public class MediaPlayerService
    {
        ...
        private static readonly System.Net.Http.HttpClient _warmupClient = new() { Timeout = TimeSpan.FromSeconds(5) };
        ...
        private static void SetupSystemMediaTransportControls()
        {
            try
            {
                ...
                systemControls.DisplayUpdater.AppMediaId = "Avora";
            }
            catch (Exception ex)
            {
        ...
        {
            if (!await _playTrackSemaphore.WaitAsync(TimeSpan.FromSeconds(10)))
            {
            ...
            _ = Task.Run(() => SendPlaybackStatistics(trackdata));
            _ = Task.Run(() => SendVKStartEvent(trackdata));
        ...
        private static async Task LoadAndPlayTrack(ExtendedAudio trackdata, TimeSpan? position)
        {
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[TrackSwitch] Loading from URL: {trackdata.audio.Title}");
                ...
                _ = Task.Run(() => FillBufferForCurrentPosition());
                ...
                // Ждём максимальные 3 секунды пока источник откроется
        ...
        private static async Task FillBufferForCurrentPosition()
        {
            try
            {
                if (iVKGetAudio == null) return;
                int currentIndex = iVKGetAudio.currentTrack != null ? (int)iVKGetAudio.currentTrack : 0;
                int totalCount = iVKGetAudio.listAudio.Count;
                if (totalCount == 0) return;

                await TrackBufferService.FillBufferAsync(
                    idx =>
                    {
                        if (idx < 0 || idx >= iVKGetAudio.listAudio.Count) return (null, null);
                        var audio = iVKGetAudio.listAudio[idx];
                        return (audio.audio.Id.ToString(), audio.audio.Url);
                    },
                    currentIndex,
                    totalCount
                );

                TrackBufferService.Cleanup();
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[TrackSwitch] FillBuffer error: {ex.Message}");
            }
        }
