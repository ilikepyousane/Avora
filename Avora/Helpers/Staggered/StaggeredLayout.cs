...
namespace Avora.WinUI.Controls;
    ...
    protected override Size MeasureOverride(VirtualizingLayoutContext context, Size availableSize)
    {
        ...
        for (int i = 0; i < context.ItemCount; i++)
        {
            ...
            if (item.Height == 0)
            {
                ...
                item.Element.Measure(new Size(Math.Max(0,(float)state.ColumnWidth), (float)availableHeight));
        ...
        for (int columnIndex = 0; columnIndex < state.NumberOfColumns; columnIndex++)
        {
            ...
            for (int i = 0; i < layout.Count; i++)
            {
                ...
                if (item.Top <= context.RealizationRect.Bottom)
                {
                    ...
                    Rect bounds = new Rect((float)itemHorizontalOffset, (float)item.Top, Math.Max(0, (float)state.ColumnWidth), (float)item.Height);
