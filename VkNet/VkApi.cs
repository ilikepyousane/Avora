...
#pragma warning restore S1104 // Fields should not have public accessibility
        ...
        /// </summary>
        private readonly SemaphoreSlim _concurrentSemaphore = new SemaphoreSlim(5, 20);

        /// <summary>
        /// Очередь временных меток запросов для контроля RPS
        /// </summary>
        private readonly ConcurrentQueue<DateTime> _requestTimestamps = new ConcurrentQueue<DateTime>();

        /// <summary>
        /// Объект блокировки для RPS
        /// </summary>
        private readonly object _rpsLock = new object();
        ...
        public int MaxConcurrentRequests
        {
            ...
            {
                if (value <= 0)
                    throw new ArgumentException("MaxConcurrentRequests must be positive", nameof(value));

                _maxConcurrentRequests = Math.Min(value, 20); // Не более 20 параллельных
            }
        }
        ...
        /// <summary>
        /// Ожидание для соблюдения RPS лимита
        /// </summary>
        ...
        private async Task<string> InvokeBaseAsync(string url, IDictionary<string, string> @params)
        {
            ...
            try
            {
                ...
                var response = await RestClient.PostAsync(new Uri(url), @params, Encoding.UTF8);

                return response.Message ?? response.Value;
            }
            finally
            {
        ...
        {
            return InvokeBaseAsync(url, @params).GetAwaiter().GetResult();
        }
        ...
        private void Initialization(IServiceProvider serviceProvider)
        {
            ...
            // Настройки компромиссного подхода
            RequestsPerSecond = 10;        // 10 запросов в секунду
            MaxConcurrentRequests = 5;      // 5 параллельных запросов
